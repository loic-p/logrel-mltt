...
appTerm′ : ∀ {F G t u Γ l l′ l″}
          ...
          ([u] : Γ ⊩⟨ l″ ⟩ u ∷ F / [F])
        → Γ ⊩⟨ l′ ⟩ t ∘ u ∷ G [ u ] / [G[u]]
appTerm′ {t = t} {Γ = Γ} {l = ¹} [F] [G[u]] (noemb (Πᵣ F G D ⊢F ⊢G A≡A [F′] [G′] G-ext))
...
appTerm′ {t = t} {Γ = Γ} {l = ⁰} [F] [G[u]] (noemb (ιx (Πᵣ F G D ⊢F ⊢G A≡A [F′] [G′] G-ext)))
         (Πₜ f d funcF f≡f [f] [f]₁) [u] =
  let ΠFG≡ΠF′G′ = whnfRed* (red D) Πₙ
      F≡F′ , G≡G′ = Π-PE-injectivity ΠFG≡ΠF′G′
      F≡idF′ = PE.trans F≡F′ (PE.sym (wk-id _))
...
app-congTerm′ : ∀ {F G t t′ u u′ Γ l l′}
          ...
          ([u≡u′] : Γ ⊩⟨ l′ ⟩ u ≡ u′ ∷ F / [F])
        → Γ ⊩⟨ l′ ⟩ t ∘ u ≡ t′ ∘ u′ ∷ G [ u ] / [G[u]]
app-congTerm′ {F′} {G′} {t = t} {t′ = t′} {Γ = Γ} {l = ⁰}
              [F] [G[u]] (noemb (ιx (Πᵣ F G D ⊢F ⊢G A≡A [F]₁ [G] G-ext)))
